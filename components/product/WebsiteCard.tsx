...
export default function WebsiteCard({ product }: WebsiteCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-xl transition-all group border border-gray-100 flex flex-col relative">
      {/* Image Section */}
      <Link href={`/digital-products/${product._id}`} className="relative h-44 sm:h-48 bg-gradient-to-br from-green-500 to-teal-600 overflow-hidden block">
        {product.thumbnailUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <Globe className="w-14 h-14 text-white/50" />
          </div>
        )}
        ...
        {/* Adsense Badge */}
        {product.isAdsenseApproved && (
          <div className="absolute bottom-2 left-2 bg-green-600 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-lg">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <Link href={`/digital-products/${product._id}`}>
          <h3 className="font-bold text-gray-900 text-base sm:text-lg mb-2 line-clamp-2 group-hover:text-green-600 transition-colors leading-snug">
            {product.title}
          </h3>
        </Link>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-gray-400 font-medium">
            ({formatNumber(product.totalReviews || 0)} reviews)
          </span>
        </div>
        ...
        <div className="mb-4">
          {product.salePrice ? (
            <div className="flex items-end gap-2">
              <span className="text-2xl sm:text-3xl font-extrabold text-green-600 leading-none">
                ${product.salePrice}
              </span>
        ...
        <button
          ...
          className="w-full py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-lg font-bold text-sm hover:from-green-700 hover:to-teal-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 mt-auto"
        >
